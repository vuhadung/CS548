...
public class PatientRepresentation extends PatientType {
	...
	public List<LinkType> getLinksTreatments() {
		...
		// goi getTreatments() cua lop cha PatientType
	}

	// tao LinkType cho Patient khi biet Id (Id duoc bieu dien bang LinkType)
	...
	public PatientRepresentation(PatientDto dto, UriInfo uriInfo) {
		this();
		this.id = PatientRepresentation.getPatientLink(dto.getId(), uriInfo);
		// co the goi vi day la phuong thuc static
	...
	public PatientDto getPatientDto() {
		...
		p.setName(this.name);
		p.setDob(this.dob);
		for (LinkType l : this.getTreatments()) {
			p.getTreatments().add(Representation.getId(l));
		}
		return p;
	}
